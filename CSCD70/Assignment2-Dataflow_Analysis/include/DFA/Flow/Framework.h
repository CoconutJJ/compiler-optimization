...
struct ValuePrinter {
        static std::string print (const TValue &V)
        {
                return "";
        }
};

template <typename TDomainElem,
          typename TValue,
          typename TMeetOp,
          typename TMeetBBConstRange,
          typename TBBConstRange,
...
class Framework {
    protected:
        ...
        using AnalysisResult_t = std::tuple<DomainIdMap_t,
                                            DomainVector_t,
        ...
        std::string stringifyDomainWithMask (const DomainVal_t &Mask) const
        {
                ...
                Strout << "{";
                CHECK (Mask.size () == DomainIdMap.size () && Mask.size () == DomainVector.size ())
                        << "The size of mask must be equal to the size of domain, but got " << Mask.size () << " vs. "
                        << DomainIdMap.size () << " vs. " << DomainVector.size () << " instead";
                for (size_t DomainId = 0; DomainId < DomainIdMap.size (); ++DomainId) {
                        if (!static_cast<bool> (Mask[DomainId])) {
                                continue;
                        }
                        Strout << DomainVector.at (DomainId) << ValuePrinter<TValue>::print (Mask[DomainId]) << ", ";
        ...
        /// @param BB
        /// @return
        virtual MeetBBConstRange_t getMeetBBConstRange (const llvm::BasicBlock &BB) const = 0;
        ...
        virtual MeetOperands_t getMeetOperands (const llvm::BasicBlock &BB) const
        {
                ...
                MeetBBConstRange_t preds = this->getMeetBBConstRange (BB);
        ...
        DomainVal_t meet (const MeetOperands_t &MeetOperands) const
        {
                ...
                DomainVal_t meet_result = DomainVal_t::top();
                for (auto operand : MeetOperands) {
                        meet_result = op (meet_result, operand);
                }
        ...
        /// @param BB
        /// @return
        virtual InstConstRange_t getInstConstRange (const llvm::BasicBlock &BB) const = 0;
        ...
        virtual bool transferFunc (const llvm::Instruction &Inst, const DomainVal_t &IDV, DomainVal_t &ODV) = 0;

        virtual AnalysisResult_t run (llvm::Function &F, llvm::FunctionAnalysisManager &FAM)
        {
