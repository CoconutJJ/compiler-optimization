...
static bool processInstruction (struct Instruction *inst)
{
        bool has_changes = false;
        switch (inst->op_code) {
        case OPCODE_ADD: {
                struct Value *fst = InstructionGetOperand (inst, 0), *snd = InstructionGetOperand (inst, 1);

                if (VALUE_IS_CONST (fst) && VALUE_IS_CONST (snd)) {
                        struct Constant *new_const =
                                ConstantCreate (AS_CONST (fst)->constant + AS_CONST (snd)->constant);
        ...
        case OPCODE_PHI: {
                ...
                if (is_constant) {
                        Value_Replace_All_Uses_With (AS_VALUE (inst), AS_VALUE (ConstantCreate (constant_value)));
